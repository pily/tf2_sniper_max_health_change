...
public OnPluginStart()
{
	...
	//CVARs
	CreateConVar("setstarthealthversion", PLUGIN_VERSION, "Set Health on Spawn version", 1|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	
	g_hStartHealth		= CreateConVar("starthealth", "1", "Is plugin enabled ? 0=No, 1+ =Yes. 2=team 1 only. 3=team 2 only", 
		1|FCVAR_NOTIFY, true, 0.0, true, 3.0);
	g_hStartHealthValue	= CreateConVar("starthealth_value", "20.0", "Value to change health to. Minimum 1.", 
		1|FCVAR_NOTIFY, true, 1.0);
	g_hStartHealthType	= CreateConVar("starthealth_type", "1.0", "When to change health : 0 = On round start, 1 = On player spawn (Default).", 
		1|FCVAR_NOTIFY, true, 0.0, true, 1.0);
...
public Event_Spawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (clientId != 0 && IsClientInGame(clientId) && IsPlayerAlive(clientId))
	{
		...
		{
			if (TF2_GetPlayerClass(clientId) == TFClass_Sniper)
    		{
				setClientHealth(clientId, value);
				TF2Attrib_SetByName(clientId, "max health additive penalty", -105.0);
			}
			else{
				TF2Attrib_RemoveByName(clientId, "max health additive penalty");
        		TF2_RegeneratePlayer(clientId);
			}
		}
	}
